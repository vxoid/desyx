...
class DeattachedMut(Mut):
  def __init__(self):
    super().__init__()

  @abstractmethod
  def _mutate_unchecked(self) -> List[str]:
    raise NotImplementedError()

  def mutate(self, og: None, min_length: int, max_length: int) -> List[str]:
    result_unchecked = self._mutate_unchecked()
    return [name for name in result_unchecked if check_for_length(name, min_length, max_length) is not None]